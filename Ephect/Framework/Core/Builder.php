...
class Builder
{
    ...
    public function buildWebcomponents(string $name, string $filename): string
    {
        ...
        try {
            ...
            // include EPHECT_ROOT . "bootstrap.php";
    ...
    public function buildWebcomponentsByHttpRequest(string $name, string $filename): string
    {
        $port = IOUtils::safeRead(CONFIG_DIR . 'dev_port') ?? '80';

        $queryString = DIRECTORY_SEPARATOR . $filename;
        ...
        $output = ob_get_clean();
        IOUtils::safeWrite(LOG_PATH . $name, $output);
